...
def main() -> None:
    ...
    print(f"Found {len(books)} billing book(s) since {args.since}")
    ...
    for yymm, path in books:
        try:
            ...
            print(f"  {yymm}  ✗ {path.name}: {exc}")
            continue
        catalog.update(rows)  # later book wins
        print(f"  {yymm}  {len(rows):>6} lines  (catalog: {len(catalog)})")
    ...
    with connect() as conn:
        ...
        for i in range(0, len(items), 2000):
            written += upsert_line_languages(cur, dict(items[i:i + 2000]), source="billing-book")
            conn.commit()
            print(f"  upserted {min(i + 2000, len(items))}/{len(items)}…")
        cur.execute("SELECT SOURCE, COUNT(*) FROM CTV_LineLanguage GROUP BY SOURCE")
        print(f"\n✓ {written} row(s) written. Catalog by source: {dict(cur.fetchall())}")
