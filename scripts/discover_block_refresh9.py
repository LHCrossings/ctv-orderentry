...
print("=" * 60)
print(
    f"{'LineID':8} {'ORA_INI_h':10} {'ORA_FIN_h':10} {'id_fascia':10} {'tp_offset_h':12} {'tp_Date'}"
)
print("-" * 80)
...
for row in line_blocks:
    ...
    if tp and tp[0] is not None:
        ...
        print(
            f"{lid:<8} {ini_h:9.2f}h {fin_h:9.2f}h {id_fascia:<10} {min_h:10.2f}h-{max_h:.2f}h  cnt={tp[2]}"
        )
    else:
        ini_h = (ora_ini or 0) / FRAMES / 3600
        fin_h = (ora_fin or 0) / FRAMES / 3600
        print(
            f"{lid:<8} {ini_h:9.2f}h {fin_h:9.2f}h {id_fascia:<10} (no trafficPalinse rows in date range)"
        )
...
if r:
    ...
    print(
        f"  Line: ORA_INI={ora_ini / FRAMES / 3600:.2f}h ORA_FIN={ora_fin / FRAMES / 3600:.2f}h  {d_from} – {d_to}  Cod_User={cod_user}"
    )
    ...
    for tp_row in cursor.fetchall():
        ...
        print(
            f"    id_fascia={tp_row[0]}  offset={tp_row[1] / FRAMES / 3600:.3f}h-{tp_row[2] / FRAMES / 3600:.3f}h{marker}"
        )
