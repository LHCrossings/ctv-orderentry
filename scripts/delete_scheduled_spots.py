...
def main():
    ...
    with connect() as conn:
        ...
        if not rows:
            print(
                f"[INFO] No scheduled spots found for contract {contract_id} between {date_from} and {date_to}."
            )
            return
        ...
        conn.commit()
        print(
            f"[DONE] Deleted {deleted} scheduled spot(s) from contract {contract_id} ({date_from} to {date_to})."
        )
