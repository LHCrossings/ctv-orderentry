...
def _confirm_start_date(order: TTOrder) -> Optional[date]:
    ...
    Returns the (possibly user-overridden) start date, or None to abort.
    Returns the parsed start date unchanged when it is comfortably in the future.
    """
    if not order.flight_start:
        return None
    earliest = _parse_date(order.flight_start)
    if earliest > date.today() + timedelta(days=1):
        return earliest   # comfortably in the future — no confirmation needed

    def _fmt(d: date) -> str:
        return f"{d.month}/{d.day}/{d.strftime('%y')}"

    print(
        f"\n  ⚠ This order starts {_fmt(earliest)} "
        f"(today is {_fmt(date.today())})."
    )
