...
def gather_threeolives_inputs(file_path: str) -> Optional[dict]:
    ...
    paid = sum(l.total_spots for l in order.lines if not l.is_bonus)
    bonus = sum(l.total_spots for l in order.lines if l.is_bonus)
    cost = sum(l.rate * l.total_spots for l in order.lines)
