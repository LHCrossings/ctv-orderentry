...
class OrderDetectionService:
    ...
    def detect_from_text(
        self, first_page_text: str, second_page_text: str | None = None
    ) -> OrderType:
        """
    ...
    def _is_scwa(self, text: str) -> bool:
        ...
        """
        return "Crossings TV Media Proposal" in text and "Sacramento County Water Agency" in text
    ...
    def _is_crispin_io(self, first_page: str, second_page: str | None) -> bool:
    ...
    def _is_daviselen(self, first_page: str, second_page: str | None) -> bool:
        """
    ...
    def _is_hl_partners(self, text: str) -> bool:
        ...
        has_location = any(
            loc in text or loc in text_upper
            for loc in ["Sacramento", "San Francisco", "SACRAMENTO", "SAN FRANCISCO"]
        )
        ...
        if has_crtv and has_estimate and has_location:
            ...
            if "CRTV-Cable" not in text:
                ...
                hl_markers = ["Send Billing", "HL Agency", "H Agency", "Agency San Francisco"]
    ...
    def split_tcaa_orders(self, full_text: str) -> list[dict[str, str]]:
        ...
        for part in parts:
            ...
            has_schedule = (
                "SCHEDULE TOTALS" in part
                or "Station Total:" in part
                or part.count("CRTV-Cable") > 3  # Has multiple line items
            )

            # Summary pages only have aggregate data
            is_summary = "Summary by Market" in part or "Summary by Station/System" in part

            # Only include sections with actual schedule data (not summaries)
            if has_schedule and not is_summary:
                sections.append({"estimate": estimate_num, "text": part})
    ...
    def _is_opad(self, text: str) -> bool:
        ...
        """
        return "Estimate:" in text and "# of SPOTS PER WEEK" in text and "KBTV" not in text
    ...
    def _is_misfit(self, text: str) -> bool:
        ...
        """
        has_misfit = (
            "Agency: Misfit" in text
            or "@agencymisfit.com" in text
            or ("Misfit" in text and "Crossings TV" in text)
        )
    ...
    def _is_impact(self, text: str) -> bool:
        ...
        """
        has_impact = (
            "Impact Marketing" in text
            or "Big Valley Ford" in text
            or "@impactcalifornia.com" in text
        )
    ...
    def _is_igraphix(self, text: str) -> bool:
        ...
        has_client = (
            "Pechanga" in text or "Sky River" in text or ("c/o" in text and "Crossings TV" in text)
        )
    ...
    def _is_rpm(self, text: str) -> bool:
        ...
        has_market = any(
            market in text
            for market in ["Seattle-Tacoma", "Sacramento-Stockton", "Sacramento", "San Francisco"]
        )
        has_estimate = "Estimate:" in text
    ...
    def extract_client_name(
        self, first_page_text: str, second_page_text: str | None, order_type: OrderType
    ) -> str | None:
        """
    ...
    def _extract_daviselen_client(self, first_page: str, second_page: str | None) -> str | None:
        """
