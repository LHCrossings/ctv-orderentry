...
class OrderProcessingService:
    ...
    def _process_tt_order(self, order: Order, shared_session=None) -> ProcessingResult:
    ...
    def _process_crispin_order(self, order: Order, shared_session=None) -> ProcessingResult:
        """Process a Crispin / Bay Area AQMD order — one contract (single market)."""
        inp = order.order_input if isinstance(order.order_input, dict) else {}
        try:
    ...
    def _process_pop_order(self, order: Order, shared_session=None) -> ProcessingResult:
        """Process a Prince of Peace sales confirmation — one contract (direct, 0%)."""
        inp = order.order_input if isinstance(order.order_input, dict) else {}
        try:
            from browser_automation.parsers.pop_parser import parse_pop
            from browser_automation.pop_automation import run_pop_order

            parsed = parse_pop(str(order.pdf_path))
            results = run_pop_order(parsed, inp)  # list of (label, success)

            contracts = [
                Contract(contract_number=label, order_type=OrderType.POP)
                for label, ok in results
                if ok
            ]
            if not contracts:
                return ProcessingResult(
                    ...
                    error_message="POP processing failed — check output above",
                )
            return ProcessingResult(success=True, contracts=contracts, order_type=OrderType.POP)

        except Exception as exc:
            import traceback

            error_detail = f"POP processing error: {exc}\n{traceback.format_exc()}"
            print(f"\n✗ POP processing failed: {exc}")
            return ProcessingResult(
                success=False,
                contracts=[],
                order_type=OrderType.POP,
