...
T_DAVIS_WA_MCD = {
    "name": "Davis Elen WA McD",
    "agency_name": "Daviselen",
    "advertiser_match": "Western WA Operator Association (McD)",
    "market_match": "SEA",
    "etere_customer_ids": [122],
}
T_DAVIS_SOCAL_TOYOTA = {
    "name": "Davis Elen SoCal Toyota",
    "agency_name": "Daviselen",
    "advertiser_match": "Southern CA Toyota Dealers Association",
    "market_match": "LAX",
    "etere_customer_ids": [362],
}
T_RPM_MUCKLESHOOT = {
    "name": "RPM Muckleshoot",
    "agency_name": "RPM Advertising",
    "advertiser_match": "Muckleshoot Casino",
    "market_match": "SEA",
...
T_RPM_TVC_CVC = {
    "name": "RPM TVC (CVC)",
    "agency_name": "RPM Advertising",
    "advertiser_match": "Thunder Valley Casino",
    "market_match": "CVC",
...
T_OCEAN_BETMGM = {
    "name": "Ocean Media BetMGM",
    "agency_name": "Ocean Media, LLC",
    "advertiser_match": "",
    "market_match": "",
    "etere_customer_ids": [999],
}
T_BVK = {
    "name": "BVK UC Davis Health",
    "agency_name": "BVK",
    "advertiser_match": "",
    "market_match": "",
...
def test_mcd_never_matches_toyota():
    m = match_template(
        ALL, customer_id=122, market="SEA", filename="2606-016 Davis Elen_2685_postlog.csv"
    )
...
def test_thunder_valley_never_matches_muckleshoot():
    m = match_template(
        ALL, customer_id=68, market="CVC", filename="2606-047 RPM Advertising Inc_2833_postlog.csv"
    )
...
def test_generic_word_media_never_pulls_betmgm():
    ...
    m = match_template(ALL, customer_id=None, filename="2606-042 Media Solutions_2763_postlog.csv")
...
def test_id_match_beats_advertiser_text():
    ...
    m = match_template(
        ALL, customer_id=122, market="SEA", advertiser="Southern CA Toyota Dealers Association"
    )
    assert m.name == "Davis Elen WA McD"
...
def test_advertiser_text_fallback_is_flagged_fuzzy():
    m = match_template(ALL, customer_id=None, market="SEA", advertiser="Muckleshoot Casino")
    assert m.name == "RPM Muckleshoot"
    assert m.confidence == "fuzzy"


def test_distinctive_agency_word_in_filename_is_fuzzy():
    m = match_template(ALL, customer_id=None, filename="2606-009 BVK_2738_postlog.csv")
...
def test_no_match_returns_none_not_first_template():
    m = match_template(
        ALL, customer_id=None, filename="2606-099 Totally Unknown Agency_9999_postlog.csv"
    )
    assert m.name == ""
    assert m.confidence == "none"
