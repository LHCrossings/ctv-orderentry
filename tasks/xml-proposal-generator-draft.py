...
def r(t): return f"{{{ROOT}}}{t}"
def vb(t): return f"{{{TVB}}}{t}"
def tp(t): return f"{{{TP}}}{t}"
...
def periods(weekly, boundaries):
    out=[]; i=0; n=len(weekly)
    while i<n:
        c=weekly[i]
        if not c or c<=0: i+=1; continue
        j=i
        while j+1<n and weekly[j+1]==c: j+=1
        out.append((boundaries[i][0], boundaries[j][1], int(c))); i=j+1
    return out

def build(spec, H):
    ET.register_namespace("",ROOT); ET.register_namespace("tvb",TVB); ET.register_namespace("tvb-tp",TP)
    root=ET.Element(r("AAAA-Message"))
    av=sub(root,r("AAAA-Values"))
    sub(av,r("SchemaName"),"SpotTVCableProposal"); sub(av,r("SchemaVersion"),"0.3.0.5A")
    sub(av,r("Media"),"SpotTV"); sub(av,r("BusinessObject"),"Proposal"); sub(av,r("Action"),"New")
    ...
    sub(p,r("Name"),H["name"]); sub(p,r("SellerReference"),H["uid"])
    outs=sub(p,r("Outlets")); sub(outs,r("TelevisionStation"),callLetters=H["call"],parentPlus="N",outletId="OUT0")
    al=sub(p,r("AvailList"),identifier="AL001",startDate=spec.flight_start,endDate=spec.flight_end,isPackage="N")
    sub(al,r("Name"),spec.market_description)
    ors=sub(al,r("OutletReferences")); sub(ors,r("OutletReference"),outletFromProposalRef="OUT0",outletForListId="OUL0")
    ...
    for ln in spec.lines:
        ...
        for dt in ln.day_times:
            d=sub(dts,r("DayTime")); sub(d,r("StartTime"),dt.start_time); sub(d,r("EndTime"),dt.end_time)
            days=sub(d,r("Days"))
            for i,nm in enumerate(DAYNAMES): sub(days,tp(nm),"Y" if dt.days[i] else "N")
        sub(awl,r("DaypartName"),ln.daypart_name or "ROS"); sub(awl,r("AvailName"),ln.program)
        ...
        for (st,en,cnt) in periods(ln.weekly_spots, spec.week_boundaries):
            ...
            sub(dp,r("Rate"),f"{ln.rate:.2f}"); sub(dp,r("SpotsPerWeek"),cnt)
    return root

o=parse_charmaine_pdf("incoming/BAAQMD-2026.pdf")[0]
o.lines=[l for l in o.lines if not l.language.strip().lower().startswith("total")]
...
if not ok:
    for e in schema.error_log[:12]: print("  -", e.message[:150])
