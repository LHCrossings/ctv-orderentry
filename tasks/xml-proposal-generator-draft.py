...
def build(spec, H):
    ...
    p = sub(
        root,
        r("Proposal"),
        uniqueIdentifier=H["uid"],
        version="1",
        sendDateTime=datetime(2026, 7, 21, 9, 0, 0).isoformat(),
        weekStartDay="Mo",
        startDate=spec.flight_start,
        endDate=spec.flight_end,
    )
...
o.lines = [
    _dc.replace(_ln, daypart=_ln.daypart.replace("11a-1p", "10a-1p"))
    if "11a-1p" in (_ln.daypart or "")
    else _ln
    for _ln in o.lines
]
spec = charmaine_order_to_proposal_spec(
    o, estimate_number="BAAQMD-2026-REV1", buyer_name="Crispin", call_letters="CRTV"
)
# DISCOUNTED rate (col E) + per-line spot length (col F) from the proposal Excel
import openpyxl

_wb = openpyxl.load_workbook(
    "/mnt/c/Work Temp/!New/!Orders/Crossings TV Media Proposal_BAAQMD_2026_REV1.xlsm",
    data_only=True,
)
_xmap = {}
for _r in _wb["BAAQMD"].iter_rows(values_only=True):
    ...
    if (
        isinstance(_lang, str)
        and isinstance(_disc, (int, float))
        and isinstance(_len, str)
        and _len.strip().rstrip("s").lstrip(":").isdigit()
    ):
...
H = dict(
    uid="BAAQMD-2026-REV1",
    name="BAAQMD 2026",
    seller="Crossings TV",
    seller_office="Sacramento",
    salesperson="Charmaine Lane",
    seller_phone="(888)901-5288 x106",
    seller_fax="(888)878-8936",
    seller_email="charmaine.lane@crossingstv.com",
    buyer="Crispin",
    buyer_office="TBD",
    buyer_name="Alexander Boyle",
    product="Bay Area AQMD",
    call="Crossings TV",
...
schema = etree.XMLSchema(
    etree.parse(".claude/documents/tvb_xml_schemas/spotTVCableProposal-0.3.0.5A.xsd")
)
