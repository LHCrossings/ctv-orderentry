...
**Rule:** An ad-hoc script issued `BEGIN TRAN` … `COMMIT` as SQL statements on
`etere_direct_client.connect()`, printed `COMMITTED` for 18 market-days, and wrote a 204-line
restore file — and the database was unchanged. pymssql opens the connection with autocommit
OFF, so it already holds an implicit transaction; the explicit `BEGIN TRAN` nested inside it
(`@@TRANCOUNT` 2), the SQL `COMMIT` only decremented the count, and `conn.close()` rolled the
outer transaction back. Every existing tool (Finish, Daily Programming) commits with
`conn.commit()` for exactly this reason. Caught only because the post-write check ran on a NEW
connection and counted 90 live pieces where 0 were expected.

**How to apply:**
1. On `connect()` connections, transaction control is `conn.commit()` / `conn.rollback()`,
   never `cur.execute("COMMIT")`. A dry run is `conn.rollback()`.
2. A script's own success print is not evidence. After ANY production write, re-read the
   target from a fresh connection and compare against the expected end state (here: pieces by
   LIVELLO, trafficPalinse count, anchors, asset count). This is the fourth appearance of
   "read back the value you meant to write" (`REBOUND 4 rows`, deploy `Already up to date`,
   Finish/DAL SUPPORTO).
3. A restore file generated by a run that did not persist is a trap for the next reader —
   delete it and regenerate on the real run.
4. Unplace recipe that worked (mirrors Finish/DP conventions): 666 the PGM pieces + Finish's
   PI/PSA/ID fill (`is_fill`: ID, or PER/PSA with contract line NULL/-1) + the hour's NOOPs,
   DELETE their trafficPalinse rows, `_rebuild` (shiftup=0) from the first live row so Etere
   re-lays the NOOP holes, verify spots unmoved / 09:00 F anchor / no NEW overlaps (a :30 break
   already overrunning the anchor by 6 frames is the spot scheduler's, not yours). Script:
   session scratchpad `unplace_korean_specials.py`; restore `logs/korean-specials-unplace-restore-20260923.sql`.
