    ...
    try:
        for cu, label, src_mon, first, last in JOBS:
            ...
            for i in range(7):
                ...
                cur.execute("""SELECT ca.ID_TrafficSchedule, ca.Notes, ca.JingleInserted,
                                      ca.blockautoinsert, ca.blockautoinsertUser
                               FROM Traffic_Calendar ca WITH(NOLOCK)
                               WHERE ca.Cod_User=%s AND ca.Date=%s AND ca.Level=0""", (cu, d))
                row = cur.fetchone()
                if not row:
                    print(f"  ABORT: source day {d} missing")
                    return False
                cur.execute("""SELECT COUNT(*) FROM traffic_scheduleblock WITH(NOLOCK)
                               WHERE ID_TrafficSchedule=%s""", (row[0],))
            ...
            cur.execute("""SELECT COUNT(*) FROM Traffic_Calendar WITH(NOLOCK)
                           WHERE Cod_User=%s AND Date BETWEEN %s AND %s""", (cu, first, last))
            n = cur.fetchone()[0]
            if n:
            ...
            for d in targets:
                ...
                cur.execute("""INSERT INTO traffic_schedule (Name,Flag,Notes,Cod_User,Insert_Date,Expired)
                               OUTPUT INSERTED.ID_TrafficSchedule
                               SELECT Name,Flag,Notes,Cod_User,Insert_Date,Expired
                               FROM traffic_schedule WHERE ID_TrafficSchedule=%s""", (s["sched"],))
                new_sched = cur.fetchone()[0]
                cur.execute("""INSERT INTO traffic_scheduleblock (ID_TrafficSchedule,ID_TrafficBlock,Locked,Offset)
                               SELECT %s, ID_TrafficBlock, Locked, Offset
                               FROM traffic_scheduleblock WHERE ID_TrafficSchedule=%s""",
                            (new_sched, s["sched"]))
                notes, jingle, bai, baiu = s["cal"]
                cur.execute("""INSERT INTO Traffic_Calendar
            ...
            for d in targets:
                ...
                if (miss or extra or nblk != s["nblk"]
                        or int(mn) != DAY_START or int(mx) - DAY_START != SPAN_24H):
            ...
            cur.execute("""SELECT COUNT(DISTINCT ca.Date), MIN(ca.Date), MAX(ca.Date)
                           FROM Traffic_Calendar ca
                           JOIN traffic_scheduleblock sb ON sb.ID_TrafficSchedule=ca.ID_TrafficSchedule
                           WHERE ca.Cod_User=%s AND ca.Level=0""", (cu,))
