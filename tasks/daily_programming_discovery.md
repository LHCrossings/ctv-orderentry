...
### Gotchas & required extra step (validated 2026-06-18 across all 9 CTV markets)
- **`Traffic_InsertEvent` `@OLD_ID` OUTPUT is NOT the new TPALINSE id** (it's a `trafficPalinse` companion id). The insert creates a `PART=0` whole-file row at the given `@ora`; recover the real id via `SELECT id_tpalinse WHERE COD_USER+DATE+ID_FILMATI+ORA+PART=0` (each part's ora is unique), then `UPDATE` it.
- `@id_operazione=0` works.
- **Close-bumper order guarantee (REQUIRED):** Etere's explode places the floating close bumper unpredictably (per-market, depends on its pasted `XORDER`). After inserting the parts, if `close.XORDER <= Part5.XORDER`, set `close.XORDER = (Part5.XORDER + next_event_XORDER)/2` so it sorts right after Part 5; *then* run the rebuild. This deterministically fixes the bug (2 of 5 fanned markets were naturally correct; 3 needed it).
- **Validated:** scripted explode + rebuild + this guarantee ran transaction-safe across HOU/SEA/LAX/CVC/WDC/MMT (commit only on 5-segment + grouped `Event_P` + close-after-Part5 match) → all correct, matching the manually-done NYC/CMP/SFO. The per-market function is the tool's core engine; the web tool just fans it across the picked network's COD_USERs.
